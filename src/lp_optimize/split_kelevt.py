...
def main(task_sel_json_file, elevt_file, output_dir):
    # 1. 从JSON文件提取时间区间
    ...
    try:
        ...
        df = pd.read_csv(elevt_file, header=None, names=['height', 'segment', 'time'])
        print(f"成功读取 {len(df)} 条记录")
    except FileNotFoundError:
        print(f"错误: 数据文件 '{elevt_file}' 未找到")
        return
    except Exception as e:
...
if __name__ == "__main__":
    ...
    args.add_argument("--elevt", required=True, type=str, default="")
    args.add_argument("--out", required=True, type=str, default="")
    params = args.parse_args()

    task_sel_path = params.task_sel
    elevt_path = params.elevt
    output = params.out

    if task_sel_path == "" or elevt_path == "" or output == "":
        exit(-1)
    if not os.path.exists(task_sel_path) or not os.path.exists(elevt_path):
        exit(-1)

    main(task_sel_json_file=task_sel_path, elevt_file=elevt_path, output_dir=output)
